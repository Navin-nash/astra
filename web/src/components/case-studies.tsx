...
function MetricStat({
  ...
  useEffect(() => {
    ...
    const ctrl = animate(0, end, { duration, ease: [0.16, 1, 0.3, 1], onUpdate: setCurrent });
    return () => ctrl.stop();
  ...
  return (
    <div className="flex flex-col gap-2 text-left p-6">
      <p ref={ref} className="text-2xl font-medium text-gray-900 dark:text-white sm:text-4xl" aria-label={`${label} ${value}`}>
        {prefix}{formatted}{suffix}
      </p>
      <p className="font-medium text-gray-900 dark:text-white text-left">{label}</p>
      {sub && <p className="text-gray-600 dark:text-gray-400 text-left">{sub}</p>}
    </div>
  );
}
